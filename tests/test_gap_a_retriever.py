...
class _NeighborGraphStore(_FakeGraphStore):
    def get_source_chunks(self, notebook_id: str, entity: str) -> List[str]:
        if entity == "CFD":
            return ["graph-chunk-1"]
        if entity == "boundary layer":
            return ["graph-neighbor-1"]
        return []

    def get_neighbors(self, notebook_id: str, entity: str, depth: int = 1) -> List[str]:
        if entity == "CFD" and depth >= 1:
...
class TestRetrieveWithGraphExpansion:
    ...
    def test_retrieve_returns_bm25_results_when_vector_search_is_empty(self):
        class _BM25OnlyIndex(_FakeBM25Index):
            ...
            def query(self, q, top_k=10, extra_tokens=None):
                return [{"text": "bm25-only content", "metadata": {"source": "bm25.pdf"}}]

        r = _make_retriever()
        r.vector_store = _EmptyVectorStore()
        r.bm25_index = _BM25OnlyIndex()

        results = r.retrieve("bm25 fallback query", top_k=5, final_k=2)
        assert [chunk["text"] for chunk in results] == ["bm25-only content"]
